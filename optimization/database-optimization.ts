  ...
  async query<T = any>(
    ...
    try {
      ...
      if (duration > 100) {
        ...
        console.warn(`[DBPool] Slow query (${duration}ms):`, text.substring(0, 100));
      }
...
async function createIndexes(pool: OptimizedPool) {
  ...
  for (const index of INDEX_DEFINITIONS) {
    ...
    try {
      ...
      console.error(`[DBOptimization] ✗ Failed to create ${indexName}:`, error.message);
    }
  }
...
async function dropIndexes(pool: OptimizedPool, indexNames: string[]) {
  ...
  for (const indexName of indexNames) {
    try {
      ...
      console.error(`[DBOptimization] ✗ Failed to drop ${indexName}:`, error.message);
    }
  }
}
