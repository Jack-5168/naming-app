...
class VirtualList {
  ...
  _render() {
    ...
    const end = Math.min(this.startIndex + this.visibleCount + 2, this.items.length);
    ...
    // 只渲染可见区域 + 缓冲区
    this.contentEl.innerHTML = visibleItems.map((item, i) => {
...
const submit = debounce(() => {
  ...
  return fetch('/api/submit', { method: 'POST' })
    .then(r => r.json())
    .finally(() => { submitBtn.disabled = false; submit.cancel(); });
...
class SessionManager {
  ...
  // ✅ 定期清理过期 session
  cleanup(maxAge = 30 * 60 * 1000) { // 30 分钟
    const now = Date.now();
...
processor.process(largeArray, async (item) => {
...
*专项完成时间: 2026-05-01 05:00 | 文件: phase2-vue3/05-performance-optimization.md*
