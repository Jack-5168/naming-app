...
import {
  calculateMBTIType,
  calculateResult,
} from "../src/services/result-calculator";
...
describe("Phase 2 Integration Tests", () => {
  ...
  describe("Stability Calculator", () => {
      ...
      const history: TestHistory[] = [
        {
          testId: "t1",
          mbtiType: "INTJ",
          dimensionScores: { E: 20, N: 70, T: 75, J: 80 },
          completedAt: Date.now() - 86400000,
        },
        {
          testId: "t2",
          mbtiType: "INTJ",
          dimensionScores: { E: 25, N: 72, T: 73, J: 78 },
          completedAt: Date.now() - 43200000,
        },
        {
          testId: "t3",
          mbtiType: "INTJ",
          dimensionScores: { E: 22, N: 68, T: 76, J: 82 },
    ...
    test("should calculate confidence band correctly", async () => {
      ...
      expect(result.confidenceBand.lower).toBeLessThanOrEqual(
        result.stabilityIndex,
      );
      expect(result.confidenceBand.upper).toBeGreaterThanOrEqual(
        result.stabilityIndex,
      );
      expect(
        result.confidenceBand.upper - result.confidenceBand.lower,
  ...
  describe("Result Calculator", () => {
    ...
    test("should calculate confidence based on score extremity", () => {
      ...
      expect(extremeResult.confidence).toBeGreaterThan(
  ...
  describe("End-to-End Test Flow", () => {
    ...
    test("should integrate stability calculation with test results", async () => {
      ...
      expect(["stable", "moderate", "unstable", "new"]).toContain(
        stability.status,
      );
    });
  });
