...
function createTestData(count: number, variance = 3): TestResult[] {
...
async function runVerification() {
  ...
  console.log("【测试 1】边界条件处理");
  {
    ...
    if (
      !result10.isRange &&
      ["stable", "evolving", "unstable"].includes(result10.status)
    ) {
  ...
  console.log("\n【测试 2】稳定性指数范围 (0-1)");
  {
    ...
    if (result.stabilityIndex >= 0 && result.stabilityIndex <= 1) {
      console.log(
        `  ✅ 稳定性指数：${result.stabilityIndex.toFixed(3)} (在 0-1 范围内)`,
      );
      passed++;
    } else {
  ...
  console.log("\n【测试 3】Monte Carlo 模拟分布");
  {
    ...
    if (result.stabilityProbability > 80) {
      console.log(
        `  ✅ 稳定数据概率：${result.stabilityProbability.toFixed(1)}% (> 80%)`,
      );
      passed++;
    } else {
  ...
  console.log("\n【测试 4】置信区间计算");
  {
    ...
    if (
      ...
      console.log(
        `     稳定性指数 ${result.stabilityIndex.toFixed(3)} 在区间内`,
      );
      passed++;
    } else {
  ...
  console.log("\n【测试 5】与真实重测数据一致性 > 80%");
  {
    ...
    if (result.stabilityIndex > 0.8) {
      console.log(
        `  ✅ 一致性指数：${result.stabilityIndex.toFixed(3)} (> 0.8)`,
      );
      passed++;
    } else {
      console.log("  ❌ 一致性不足");
  ...
  console.log("\n【测试 6】置信区间覆盖率 > 95%");
  {
    ...
    for (let i = 0; i < trials; i++) {
      ...
      if (
        result.stabilityIndex >= result.confidenceBand[0] &&
  ...
  console.log("\n【测试 7】警告信息触发条件");
  {
    ...
    if (
      result2.stabilityWarning &&
      result2.stabilityWarning.includes("测试次数不足")
    ) {
      console.log("  ✅ < 3 次触发警告");
      passed++;
    } else {
  ...
  console.log("\n【测试 9】并发支持 > 50 QPS (TURBO 模式)");
  {
    ...
    if (qps > 50) {
      ...
      console.log(
        `     总时间：${time}ms, 平均：${(time / 50).toFixed(1)}ms/计算`,
      );
      passed++;
    } else {
