...
async function testEvolving() {
  ...
  console.log(
    `置信区间：[${result.confidenceBand.lower.toFixed(3)}, ${result.confidenceBand.upper.toFixed(3)}]`,
  );
  console.log(`警告：${result.stabilityWarning}`);
  console.log("");
}
...
async function testStable() {
  ...
  for (const dim of dims) {
    ...
    if (stats) {
      console.log(
        `  ${dimNames[dim]}: 均值=${stats.mean?.toFixed(1) ?? "-"}, 标准差=${stats.std?.toFixed(2) ?? "-"}, CV=${stats.cv?.toFixed(3) ?? "-"}`,
      );
    }
  }
