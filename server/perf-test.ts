...
async function runPerfTest() {
  ...
  const optimized = new StabilityCalculatorOptimized({
  ...
  // 标准版
  const standardResult = await benchmark("标准版 (单次)", async () => {
    await standard.calculateStability(1, testData);
  });

  // 优化版
  const optimizedResult = await benchmark("优化版 (单次)", async () => {
  ...
  // 标准版并发
  const standardConcurrent = await benchmark("标准版 (并发 10)", async () => {
    await Promise.all(
  ...
  // 优化版并发
  const optimizedConcurrent = await benchmark("优化版 (并发 10)", async () => {
    await Promise.all(
      Array.from({ length: 10 }, (_, i) =>
  ...
  // 性能提升
  const improvement =
    ((standardResult.avg - optimizedResult.avg) / standardResult.avg) * 100;

  console.log("=== 性能提升 ===");
  console.log(`单次计算：${improvement.toFixed(1)}% 提升`);

  const concurrentImprovement =
    ((standardConcurrent.avg - optimizedConcurrent.avg) /
      standardConcurrent.avg) *
    100;
