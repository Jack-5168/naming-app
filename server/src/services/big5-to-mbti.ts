...
export function mapBig5ToMBTI(
  ...
  const overallConfidence =
    (E_I_confidence + N_S_confidence + T_F_confidence + J_P_confidence) / 4;
...
function calculateConfidence(
  ...
  if (distanceFrom50 <= threshold.moderate) {
    // 中等倾向：置信度 0.5-0.7
    return (
      0.5 +
      ((distanceFrom50 - threshold.balanced) /
        (threshold.moderate - threshold.balanced)) *
        0.2
    );
  }

  if (distanceFrom50 <= threshold.strong) {
    // 明显倾向：置信度 0.7-0.9
    return (
      0.7 +
      ((distanceFrom50 - threshold.moderate) /
        (threshold.strong - threshold.moderate)) *
        0.2
    );
  }
  ...
  // 强烈倾向：置信度 0.9-1.0
  return (
    0.9 + ((distanceFrom50 - threshold.strong) / (50 - threshold.strong)) * 0.1
  );
}
