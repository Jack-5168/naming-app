 ...
 */
function extractDimensionScores(answers: any): {
  E: number;
  N: number;
  T: number;
  J: number;
...
function calculateOverallScore(
  ...
  const typeInteractionBonus = getTypeInteractionBonus(
    initiatorType,
    participantType,
  );
 ...
 */
function generateConflictWarnings(
  dimensionAnalysis: DimensionCompatibility[],
): ConflictWarning[] {
...
function createConflictWarning(dim: DimensionCompatibility): ConflictWarning {
  const level =
    dim.difference > 60 ? "high" : dim.difference > 40 ? "medium" : "low";

  const warningMap: {
    [key: string]: { description: string; suggestion: string };
  } = {
    E: {
 ...
 */
function getConflictResolutionAdvice(
  dimensionAnalysis: DimensionCompatibility[],
): string {
  const challengingDimensions = dimensionAnalysis.filter(
    (d) => d.compatibility === "challenging",
  );
