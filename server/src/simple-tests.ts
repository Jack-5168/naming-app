...
export async function getNextQuestion(req: any, res: any) {
  ...
  console.log('GET:', session_id, 'found:', !!session, 'size:', sessionStore.size);
  
  if (!session) return res.json({question:{id:'q1',text:'测试问题?',options:[]}});
  
  session.idx = (session.idx || 0) + 1;
  sessionStore.set(session_id, session);
  
  res.json({
    question: {id:'q'+session.idx, text:'问题'+session.idx, options:['A','B','C']},
