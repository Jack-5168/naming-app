 ...
 */
export async function generatePersonalityShareCard(
  req: Request,
  res: Response,
) {
  try {
    ...
    if (testCount > 1) {
      const { calculateStability } =
        await import("../services/stability-calculator");
      ...
      const stabilityResult = await calculateStability(
        userId.toString(),
        testHistory as any,
      );
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        sharesByChannel: sharesByChannel.reduce(
          (acc, item) => {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        reward:
          inviteCode.type === "referral"
            ? "Both you and the inviter will receive rewards"
            : null,
      },
    });
  } catch (error) {
 ...
 * Get personality type info
 */
function getPersonalityTypeInfo(type: string): {
  name: string;
  description: string;
} {
