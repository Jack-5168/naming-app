...
exports.createPresetExperiments = createPresetExperiments;
exports.createAndStartExperiment = createAndStartExperiment;
/**
 * A/B 测试管理器
 */
class ABTestingManager {
     ...
     * 记录事件
     */
    recordEvent(experimentId, userId, variantId, metricId, value) {
     ...
     * 获取用户的实验分配
     */
    getUserExposure(experimentId, userId) {
        const exposureList = this.exposures.get(experimentId) || [];
     ...
     * 找到最佳变体
     */
    findBestVariant(variantResults, metrics) {
        // 简化：选择第一个变体
        // 实际应该综合所有显著指标的表现
 ...
 */
function createPresetExperiments() {
    return [
        {
 ...
 * 创建实验并立即启动
 */
function createAndStartExperiment(manager, experimentId) {
    ...
    // 更新状态为运行中
    const config = { ...preset, status: 'running' };
    return manager.createExperiment(config);
}
exports.default = ABTestingManager;
