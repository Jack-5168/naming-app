...
exports.calculateTestRetestReliability = calculateTestRetestReliability;
exports.getStabilityStatusColor = getStabilityStatusColor;
exports.getStabilityStatusText = getStabilityStatusText;
 ...
 * 计算单个维度的稳定性统计
 */
function calculateDimensionStats(scores) {
    if (scores.length === 0) {
 ...
 * @returns 稳定性结果
 */
async function calculateStability(userId, testHistory) {
    ...
    const simulationResult = await runMonteCarloSimulation(dimensionScores, config.monteCarloIterations, config.confidenceLevel);
    // 确定状态
    let status;
    ...
    // 生成显示文本
    let stabilityProbabilityDisplay;
    if (testHistory.length < config.minTestsForStable) {
        // 范围显示
 ...
 * 通过重采样估计稳定性概率的分布
 */
async function runMonteCarloSimulation(dimensionScores, iterations, confidenceLevel) {
    // 合并所有维度分数
    const allScores = [
        ...dimensionScores.O,
        ...dimensionScores.C,
 ...
 * 使用组内相关系数 (ICC)
 */
function calculateTestRetestReliability(testHistory) {
 ...
 * 获取稳定性状态的颜色标识
 */
function getStabilityStatusColor(status) {
    switch (status) {
        case 'stable':
            return '#22c55e'; // green
        case 'evolving':
 ...
 * 获取稳定性状态的中文描述
 */
function getStabilityStatusText(status) {
    switch (status) {
        case 'stable':
            return '稳定';
        case 'evolving':
...
exports.default = calculateStability;
//# sourceMappingURL=stability-calculator.js.map