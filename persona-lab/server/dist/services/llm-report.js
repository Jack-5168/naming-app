...
exports.reportEvents = void 0;
exports.generateReport = generateReport;
exports.getRateLimitStatus = getRateLimitStatus;
exports.resetRateLimit = resetRateLimit;
const events_1 = require("events");
const cost_control_1 = require("./cost-control");
// 事件发射器
exports.reportEvents = new events_1.EventEmitter();
 ...
 * 增加限流计数
 */
function incrementRateLimit(userId, clientIp) {
...
function renderTemplate(template, data) {
    ...
    if (data.lifeEvents && Array.isArray(data.lifeEvents)) {
        ...
        if (sectionMatch) {
            ...
            const items = data.lifeEvents.map((event) => {
 ...
 * 质量验证
 */
function validateQuality(content, reportType) {
    const issues = [];
    // 字数检查
 ...
 * 计算成本（人民币）
 */
function calculateCost(tokens, model, inputRatio = 0.3) {
...
async function generateReport(params) {
    ...
    exports.reportEvents.emit('reportStarted', { requestId, params });
    try {
        // 1. 检查限流
        ...
        // 2. 获取模型策略
        const strategy = (0, cost_control_1.getModelStrategy)(params.reportType);
        // 3. 检查预算
        const estimatedCost = calculateCost(strategy.maxTokens, strategy.model);
        if (!(0, cost_control_1.isWithinBudget)(estimatedCost)) {
            const status = (0, cost_control_1.getBudgetStatus)();
        ...
        let lastError = null;
        let result = null;
        const maxRetries = (0, cost_control_1.getMaxRetries)();
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                lastError = error;
                exports.reportEvents.emit('reportRetry', { requestId, attempt, error: error.message });
                if (attempt === maxRetries) {
                    // 尝试降级
                    if ((0, cost_control_1.getFallbackModel)()) {
                        exports.reportEvents.emit('reportFallback', { requestId, from: strategy.model, to: (0, cost_control_1.getFallbackModel)() });
                        try {
                            result = await callLLM(prompt, (0, cost_control_1.getFallbackModel)(), strategy.maxTokens);
                        }
                        catch (fallbackError) {
                            // 降级也失败
                        }
        ...
        if (!validation.valid) {
            exports.reportEvents.emit('reportQualityWarning', { requestId, issues: validation.issues });
            // 不抛出错误，仅记录警告
        }
        // 8. 计算实际成本
        ...
        exports.reportEvents.emit('reportCompleted', { requestId, result: reportResult });
        return reportResult;
    }
    catch (error) {
        exports.reportEvents.emit('reportFailed', { requestId, error: error.message });
        throw error;
    }
}
/**
 * 获取限流状态
 */
function getRateLimitStatus(userId, clientIp) {
    return checkRateLimit(userId, clientIp);
}
/**
 * 重置限流（管理员功能）
 */
function resetRateLimit(userId, clientIp) {
    if (userId) {
...
exports.default = {
    ...
    getRateLimitStatus,
    resetRateLimit,
    reportEvents: exports.reportEvents,
    BASIC_REPORT_TEMPLATE,
    PRO_REPORT_TEMPLATE,
    MASTER_REPORT_TEMPLATE
};
//# sourceMappingURL=llm-report.js.map