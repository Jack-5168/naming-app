...
exports.upgradeMembership = upgradeMembership;
exports.getMembershipBenefits = getMembershipBenefits;
 ...
 */
async function getMembershipProducts(req, res) {
    try {
 ...
 */
async function getMembershipBenefits(req, res) {
    try {
        const { level } = req.query;
        if (level) {
            const benefits = await getBenefitsForLevel(level);
            return res.json({
                success: true,
                data: {
                    level,
        ...
        const allBenefits = await Promise.all(levels.map(l => getBenefitsForLevel(l)));
        res.json({
            success: true,
 ...
 */
async function getBenefitsForLevel(level) {
    const benefitConfig = {
        free: {
            report_basic: { unlimited: false, limit: 1 },
            report_pro: { unlimited: false, limit: 0 },
 ...
 */
async function recordFeatureUsage(userId, feature, metadata) {
