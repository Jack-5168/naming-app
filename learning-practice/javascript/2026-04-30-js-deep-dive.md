# 🔬 JavaScript 深度专项 — 第五轮进阶：闭包/原型/异步/事件循环（综合实战版）
...
const timerDecorator = (method, result, args, ctx) => {
  ...
  const res = result instanceof Promise ? 
    result.then(r => { console.log(`[${method}] ${performance.now() - start}ms`); return r; }) :
...
const decoratedCalc = createDecoratedObject(calculator, loggerDecorator, timerDecorator);
...
class EventLoopSimulator {
  ...
  run() {
    this.running = true;
    while (this.running && (this.microtasks.length > 0 || this.macrotasks.length > 0)) {
...
const processData = asyncPipeline(
  ...
  async (data) => ({ ...data, words: data.cleaned.split(/\s+/).filter(Boolean) })
);
...
class Scheduler {
  ...
  schedule(task, priority = 'normal') {
    const queue = priority === 'immediate' ? this.immediateQueue :
                  priority === 'low' ? this.lowQueue : this.normalQueue;
    queue.push(task);
    this._flush();
  }
...
*2026-04-30 01:00 | JavaScript.info 第 5-7 章深度专项*
