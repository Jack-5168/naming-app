...
> = K extends readonly [infer First, ...infer Rest]
  ? First extends string | number | symbol
    ? {
        [P in First]: Rest extends readonly any[]
...
// 柯里化类型推导
type Curried<Fn extends (...args: any[]) => any> =
  RestArgs<Fn> extends []
    ? () => ReturnType<Fn>
...
```typescript
type DeepFlat<T extends readonly any[]> = T extends readonly [
  infer First,
  ...infer Rest,
]
...
> = Acc["length"] extends Length
  ? Acc
  : BuildTuple<Length, Element, [...Acc, Element]>;
...
// 2. 三段匹配（替换的核心）
type Replace<S, From, To> = S extends `${infer B}${From}${infer A}`
  ? `${B}${To}${A}`
  : S;
