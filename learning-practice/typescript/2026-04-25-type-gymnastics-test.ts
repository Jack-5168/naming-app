...
type DeepReadonly<T> = T extends Primitive ? T : T extends (...args: any[]) => any ? T : { readonly [P in keyof T]: DeepReadonly<T[P]> };
...
interface Todo9 { title: string; description: string; completed: boolean; }
...
type Flatten<T extends readonly any[]> = T extends readonly [infer First, ...infer Rest]
  ? First extends readonly any[] ? [...Flatten<First>, ...Flatten<Rest>] : [First, ...Flatten<Rest>]
  : [];
...
type DeepPartial<T> = T extends Primitive ? T : T extends Array<infer U> ? Array<DeepPartial<U>> : { [P in keyof T]?: DeepPartial<T[P]> };
...
type OptionalKeysPrecise<T> = { [P in keyof T]: {} extends Pick<T, P> ? P : never; }[keyof T];
interface Ex12 { required: string; optional?: number; }
...
type CamelToKebab<S extends string> = S extends `${infer First}${infer Rest}`
  ? First extends Uppercase<First> ? `-${Lowercase<First>}${CamelToKebab<Rest>}` : `${First}${CamelToKebab<Rest>}`
  : S;
type CK1 = CamelToKebab<'camelCase'>;
const ck1: CK1 = 'camel-case';

// Challenge 15: LastParam & AllButLast
type LastParam<Fn extends (...args: any[]) => any> = Fn extends (...args: infer P) => any ? P extends [...any[], infer Last] ? Last : never : never;
type AllButLast<Fn extends (...args: any[]) => any> = Fn extends (...args: infer P) => any ? P extends [...infer Init, any] ? (...args: Init) => any : Fn : Fn;
