...
function logAndReturn(x: number) {
  console.log(x);
  return x;
} // ❌ 有副作用
```
...
const lowerEmail = (user: User): User => ({
  ...user,
...
// 柯里化的验证器
const validate =
  (rule: (val: any) => boolean, message: string) => (value: any) =>
    rule(value) ? { valid: true } : { valid: false, message };
...
// 验证管道
const validateField =
  (validators: ReturnType<typeof validate>[]) => (value: any) =>
    validators.reduce((result, v) => (result.valid ? v(value) : result), {
      valid: true,
...
const getPending = (todos: Todo[]): Todo[] => todos.filter((t) => !t.completed);
...
// 组合不同管道
const formatTitle = pipe(trim, removeExtraSpaces, capitalize, truncate(50));

const formatSearchQuery = pipe(trim, lower, removeExtraSpaces);
...
console.log(
  formatDisplay("  this is a very long title that needs truncation  "),
);
...
// 柯里化的查询条件
const where =
  (field: string, operator: string, value: any) => (query: string) =>
    `${query} WHERE ${field} ${operator} '${value}'`;

const and = (field: string, operator: string, value: any) => (query: string) =>
...
const limit = (n: number) => (query: string) => `${query} LIMIT ${n}`;
...
const activeAdultUsers = buildQuery("SELECT * FROM users", [
  where("age", ">=", 18),
  where("status", "=", "active"),
  orderBy("name", "ASC"),
  limit(20),
...
// 模拟请求
app({ method: "GET", path: "/api", token: "abc123", ip: "127.0.0.1" }).then(
  () => console.log("Response:", (ctx) => ctx.body),
);

// 无 token 请求
app({ method: "GET", path: "/api", ip: "127.0.0.1" }).then(() =>
  console.log("Response:", (ctx) => ctx.body),
);
```
...
const company: Company = {
  departments: [
    ...
    {
      name: "Marketing",
      employees: [{ id: 3, name: "Charlie", salary: 9000 }],
    },
  ],
};
...
const transformTypes =
  (schema: Record<string, (v: string) => any>) =>
...
const required =
  ...
  (data) =>
    fields.filter((f) => !data[f]).map((f) => `${f} 是必填项`);
...
const F = {
  ...
  curry<T extends (...args: any[]) => any>(fn: T): T {
    return ((...args: any[]) =>
      args.length >= fn.length
        ? fn(...args)
        : F.curry(fn.bind(null, ...args))) as T;
  },
  ...
  // 条件执行
  when:
    <T>(predicate: (x: T) => boolean, fn: (x: T) => T) =>
    (x: T) =>
      predicate(x) ? fn(x) : x,
...
_专项训练完成。12 个示例覆盖：纯函数(1,9)、不可变性(3,11)、函数组合(4,7,10)、柯里化(2,7)、高阶函数(8,12)、Functor/Monad(5,6)、ETL 管道(12)。_
