...
console.log(
  `95% CI: [${finalEstimate.confidenceInterval[0]}, ${finalEstimate.confidenceInterval[1]}]`,
);
...
for (const [dim, estimate] of Object.entries(dimensionEstimates)) {
  if (estimate) {
    console.log(
      `${dim}: θ=${estimate.theta.toFixed(2)}, SEM=${estimate.sem.toFixed(3)}`,
    );
  }
}
```
...
1. Embretson, S. E., & Reise, S. P. (2000). _Item Response Theory for Psychologists_.
2. Wirth, R. J., & Edwards, M. C. (2007). Item factor analysis: Current approaches and future directions. _Psychological Methods_.
