...
interface UseFormReturn<T> {
  ...
  /** 注册字段 */
  register: (name: string, options?: { validate?: FieldValidator }) => {
    value: any;
  ...
  /** 提交处理 */
  handleSubmit: (onValid: SubmitHandler<T>, onInvalid?: ErrorHandler) => (e?: any) => void;
  /** 手动设置错误 */
...
const TeamForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <FieldArray name="members" render={(field, index) => (
        <div key={index}>
  ...
  return (
    <DataTable
      ...
      columns={[
        ...
        { key: 'email', title: '邮箱', dataIndex: 'email', sortable: true, width: 200 },
        {
          key: 'role', title: '角色', dataIndex: 'role',
          filterable: true,
          filterOptions: [{ label: '管理员', value: 'admin' }, { label: '用户', value: 'user' }],
        },
        {
      ...
      <DataTable
        ...
        selection={{
          ...
          getProps: (record) => ({
            ...
            tooltip: record.status === 'locked' ? '已锁定，不可选择' : undefined,
          }),
        }}
      />
    </>
...
const BigDataTable = () => (
  <DataTable
    ...
    columns={[
      ...
      { key: 'name', title: '名称', dataIndex: 'name', fixed: 'left', width: 150 },
      // ... 大量列
      { key: 'lastCol', title: '操作', fixed: 'right', render: () => <ActionMenu /> },
    ]}
    pagination={false}
  />
);
...
interface UsePaginationOptions<T> {
  ...
  dataSource?: T[];
  request?: (params: { page: number; pageSize: number; sort?: Sort; filter?: Filter }) => Promise<{
    data: T[];
    total: number;
  }>;
...
const PaginatedUserList = () => {
  const { data, total, current, pageSize, loading, pageList, goTo, changePageSize, refresh } =
    usePagination({
  ...
  return (
    <>
      ...
      <div className="pagination-footer">
        ...
        <button disabled={current === 1} onClick={() => goTo(current - 1)}>上一页</button>
        <span>{current} / {Math.ceil(total / pageSize)}</span>
        <button disabled={current >= Math.ceil(total / pageSize)} onClick={() => goTo(current + 1)}>下一页</button>
      </div>
    </>
  );
};
...
const DataTableWithPagination = () => {
  const { data, loading, total, current, pageSize, goTo, changePageSize, refresh } =
    usePagination({
...
const AdvancedPaginatedList = () => {
  ...
  const { data, loading, current, goTo, refresh } =
    usePagination({
      request: async ({ page, pageSize }) =>
