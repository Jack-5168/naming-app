  ...
  try {
    ...
    if (!session) {
      return res.json({
        ...
        progress:{current:1,total:25},note:'starting fresh'
      });
    }
    ...
    res.json({
      question: {id:q.id,text:q.text,dim:q.dim,opts:q.opts.map(o=>({id:v16_encode(o.t),text:o.t}))},
...
app.post('/sessions/:id/answer', async (req, res) => {
  try {
    ...
    if (cur >= 25) {
      await prisma.session.update({
        where:{id},
        data:{answers,currentQuestion:cur,state:'completed',completedAt:new Date()}
      });
    ...
    res.json({
      question: {id:nextQ.id,text:nextQ.text,dim:nextQ.dim,opts:nextQ.opts.map(o=>({id:v16_encode(o.t),text:o.t}))},
...
function calculateResult(session) {
  ...
  const highDims = Object.entries(scores).sort((a,b)=>b[1]-a[1]).slice(0,3).map(x=>names[x[0]]).join(', ');
