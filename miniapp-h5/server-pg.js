...
pool
  .query('SELECT 1')
  .then(() => console.log('[INFO] DB connected'))
...
app.get('/sessions/:id/next', async (req, res) => {
  try {
    ...
    if (cur > 25) {
      await pool.query(
        'UPDATE sessions SET state=\'completed\',completed_at=NOW() WHERE id=$1',
        [id],
      );
      return res.json({ done: true, result: calcScore(s.answers) });
    }

    await pool.query('UPDATE sessions SET current_question=$1 WHERE id=$2', [
      cur,
      id,
    ]);
    ...
    res.json({
      question: {
        id: q.id, dim: q.dim, text: q.txt, opts: OPS.slice(),
      },
...
app.post('/sessions/:id/answer', async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body || {};
    const opt = req.body?.opt || req.body?.opt;
    const qid = req.body?.qid || req.body?.q || req.body?.qid;
