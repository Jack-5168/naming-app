...
app.get('/sessions/:id/next', async (req, res) => {
  ...
  const s = r.rows[0];
  if(!s) return res.json({q:{id:1,d:'O',t:QUESTIONS[0].t,opts:OPTS},p:{c:1,t:25}});
  if(s.state==='done') return res.json({done:true,res:calc(s.answers)});
  const cur = (s.current_question||0) + 1;
  res.json({q:{id:QUESTIONS[cur-1].id,d:QUESTIONS[cur-1].d,t:QUESTIONS[cur-1].t,opts:OPTS},p:{c:cur,t:25}});
});
...
app.post('/sessions/:id/answer', async (req, res) => {
  ...
  const ans = (s.answers?s.answers:[]);ans.push({d:qi.d,s:sc});
  if(cur>=25){
    await pool.query('UPDATE sessions SET state=done,answers=$1 WHERE id=$2',[JSON.stringify(ans),req.params.id]);
    return res.json({done:true,res:calc(ans)});
  }
  await pool.query('UPDATE sessions SET current_question=$1,answers=$2 WHERE id=$3',[cur,JSON.stringify(ans),req.params.id]);
  res.json({q:{id:QUESTIONS[cur].id,d:QUESTIONS[cur].d,t:QUESTIONS[cur].t,opts:OPTS},p:{c:cur+1,t:25}});
});

function calc(ans){
  const sc={O:0,C:0,E:0,A:0,N:0},cn={O:0,C:0,E:0,A:0,N:0};
  (ans||[]).forEach(a=>{if(a.d&&sc[a.d]!=null){sc[a.d]+=a.s;cn[a.d]++}});
  const pt={};for(let k in cn)pt[k]=Math.round((sc[k]/cn[k])/4*100);
  let mt='';mt+=pt.E>=50?'E':'I';mt+=pt.O>=50?'N':'S';mt+=pt.C>=50?'J':'P';mt+=pt.A>=50?'F':'T';
