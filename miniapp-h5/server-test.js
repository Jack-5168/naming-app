...
app.get('/sessions/:id/next', async (req, res) => {
  try {
    ...
    if (r.rows.length === 0) {
      return res.json({
        question: {
          id: 1, dim: 'O', text: Q[0].txt, options: OPS,
        },
    ...
    res.json({
      question: {
        id: q.id, dim: q.d, text: q.txt, options: OPS,
      },
...
app.post('/sessions/:id/answer', async (req, res) => {
  try {
    ...
    res.json({
      question: {
        id: nextQ.id, dim: nextQ.d, text: nextQ.txt, options: OPS,
      },
...
function calcResults(answers) {
  ...
  return {
    mbti: mt,
    scores: percent,
    summary: `Top traits: ${Object.entries(percent)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map((x) => x[0])
      .join(', ')}`,
