...
type Test9 = Expect<
  Equal<MyOmit<{ name: string; age: number }, "age">, { name: string }>
>;
...
// ============================================
type AppendArgument<T extends (...args: any[]) => any, A> = T extends (
  ...args: infer P
) => infer R
  ? (...args: [...P, A]) => R
  : never;
...
type Test13 = Expect<
  Equal<
    AppendArgument<(a: number) => string, boolean>,
...
type Replace<
  ...
  S extends string,
  R extends string,
> = T extends `${infer Prefix}${S}${infer Suffix}`
  ? `${Prefix}${R}${Suffix}`
  : T;
...
type Test19 = Expect<
  Equal<Required<Test19Result>["a"], { b?: { c?: string } }>
>;
...
type Test20 = Expect<
  Equal<
    Merge<{ a: number; b: string }, { b: boolean; c: number }>,
...
// ============================================
type DropFirst<T extends readonly any[]> = T extends readonly [
  any,
  ...infer Rest,
]
  ? Rest
...
// ============================================
type Includes<T extends readonly any[], V> = T extends readonly [
  infer F,
  ...infer Rest,
]
